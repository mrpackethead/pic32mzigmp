...
void APP_Tasks ( void )
{
    ...
    static UDP_PORT     ACNListenPort = 5568;      // server listening port
    ...
    switch(appData.state)
    {
        ...
        case APP_TCPIP_WAIT_INIT:
         ...
         case APP_JOIN_IGMP:
            ...
            appSkt =TCPIP_UDP_ServerOpen(IP_ADDRESS_TYPE_IPV4, ACNListenPort, 0);
                          
            netH= TCPIP_STACK_NetHandleGet("PIC32INT");
            
            if(TCPIP_STACK_NetIsReady(netH))
            {
                SYS_CONSOLE_PRINT("Network is ready for IGMP Join \r\n");
    
                TCPIP_IGMP_RESULT igmp_result;
                IPV4_ADDR multicast_group_add;
                TCPIP_NET_HANDLE hNet = TCPIP_STACK_NetHandleGet("PIC32INT");
       
    
                multicast_group_add.v[0]=239;
                multicast_group_add.v[1]=254;
                multicast_group_add.v[2]=0;
                multicast_group_add.v[3]=1;

  
       
               // igmp_result = TCPIP_IGMP_Join(appSkt, hNet, multicast_group_add);
    
                if (igmp_result == TCPIP_IGMP_OK)
                {
                    SYS_CONSOLE_PRINT("\r\n IGMP Join ok %i\r\n",igmp_result);
                }    
                else
